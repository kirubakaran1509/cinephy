...
export const api = {
  ...
  movies: {
    detail: (id) => axios.get(BASE + '/movie/' + id),
    search: (q)  => axios.get(BASE + '/movies/search', { params: { q } }),
    top: (n = 10, genre = null) =>
      axios.get(BASE + '/movies/top', { params: genre ? { n, genre } : { n } }),
  },
  user: {
    profile:       (user_id) => axios.get(BASE + '/user/' + user_id),
    getHistory:    (user_id) => axios.get(BASE + '/user/history/' + user_id),
    saveHistory:   (data)    => axios.post(BASE + '/user/history', data),
    deleteHistory: (user_id) => axios.delete(BASE + '/user/history/' + user_id),
